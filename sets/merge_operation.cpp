...
void customized_insertion(struct List3 *prev, struct List3 *add, int value)
{
    struct List3 *temp = (struct List3 *)malloc(sizeof(struct List3));
    temp->data3 = value;
    if (prev == NULL)
...
{
    struct List3 *curr3 = start3;
    struct List3 *prev3 = NULL;
    while (curr3 != NULL)
    {
        if (value < curr3->data3)
        {
            customized_insertion(prev3, curr3, value);
            break;
        }
        prev3 = curr3;
        curr3 = curr3->next3;
